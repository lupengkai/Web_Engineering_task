## Introduction
&#160; &#160; &#160; &#160;此次项目主要为解决大学生出行问题，而开发一个安全、方便的拼车Web应用。在初期，我们会在此版本为用户提供信息匹配、用户交流等功能。随着用户数的增多我们将推出安卓和苹果端的ＡＰＰ给用户，增加账户注册等功能。通过使用此款拼车软件，在郊区的我们可以安全便捷的出行。
...
## Letter Proposal
1. 项目是为了向学生提供出行的拼车信息，所以应当重视学生的体验。而且应当结合学生的心理，比如学生不喜欢广告。所以，网站要尽量不以插入广告赢利。
2. 考虑到注册用户一般是需要出行时使用，注册时应当告诉用户手机号码等信息会被公开为了让同行同学进行及时的沟通
3. 消息的推送应当及时，后台程序应当注重算法，不能让用户等待过久
4. 对于司机身份的验证一定要重视，防止被犯罪人员利用项目本身漏洞而造成不好的社会影响
5. 应用界面应当简洁，使用便捷，无需太多复杂功能，可参考uber
